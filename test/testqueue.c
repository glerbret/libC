...
int TstQueue(void)
{
    ...
    {
        queue1 = QUEUE_Create(&error);
        if(queue1 == NULL || error != QUEUE_NO_ERROR)
        {
        ...
        queue2 = QUEUE_Create(&error);
        if(queue2 == NULL || error != QUEUE_NO_ERROR)
        {
            printf("(QUEUE_Create) Test creation d'une file KO\n");
            return 1;
        }
    ...
    {
        cloneQueue1 = QUEUE_Clone(queue1, &error);
        if(cloneQueue1 == NULL || error != QUEUE_NO_ERROR)
        {
        ...
        cloneQueue2 = QUEUE_Clone(queue2, &error);
        if(cloneQueue2 == NULL || error != QUEUE_NO_ERROR)
        {
    ...
    {
        data = QUEUE_Peek(cloneQueue1, &error);
        if(data == NULL || error != QUEUE_NO_ERROR)
        {
        ...
        data = QUEUE_Peek(queue2, &error);
        if(data == NULL &&  error == QUEUE_EMPTY_QUEUE)
        {
    ...
    {
        data = QUEUE_Dequeue(cloneQueue1, &error);
        if(data == NULL || error != QUEUE_NO_ERROR)
        {
        ...
        data = QUEUE_Dequeue(queue2, &error);
        if(data == NULL &&  error == QUEUE_EMPTY_QUEUE)
        {
    ...
    {
        error = QUEUE_Remove(cloneQueue1);
        ...
        error = QUEUE_Remove(queue2);
        if(error == QUEUE_EMPTY_QUEUE)
        {
