/**
 * @author Gregory Lerbret
 * @file
 * @brief Gestion des chaines de caracteres
 * @date 06/04/2005
 * @version 1.0.0
 */

#ifndef GL_STR_H
#define GL_STR_H

#include <string.h>

#ifdef __cplusplus
extern "C"
{
#endif

/**
 * @return Identifiant du module (nom et version)
 */
const char* STR_Identifier(void);

/**
 * @return Version du module
 */
int STR_Version(void);

/**
 * @brief Suppression d'un caractere donne dans une chaine
 *
 * @param[out] pcDst Chaine apres traitement
 * @param[in] pcSrc Chaine initiale
 * @param[in] szLgDst Taille maximale de la chaine apres traitement
 * @param[in] cChar2Delete Caractere a supprimer

 * @return Chaine sans le caractere a supprimer (NULL en cas d'erreur).
 */
char* STR_DelChar(char* pcDst, const char* pcSrc, size_t szLgDst, char cChar2Delete);

/**
 * @brief Suppression des espaces en debut et en fin de chaine
 *
 * @param[out] pcDst Chaine sans espace superflu
 * @param[in] pcSrc Chaine initiale
 * @param[in] szLgDst Taille maximale de la chaine sans espace
 *
 * @return Chaine sans espace superflu (NULL en cas de probleme)
 */
char* STR_Strip(char* pcDst, const char* pcSrc, size_t szLgDst);

/**
 * @brief Conversion de la chaine en minuscule
 *
 * @param[out] pcDst Chaine en minuscule
 * @param[in] pcSrc Chaine initiale
 * @param[in] szLgDst Taille maximale de la chaine en minuscule
 *
 * @return Chaine en minuscule (NULL en cas de probleme)
 */
char* STR_String2Lower(char* pcDst, const char* pcSrc, size_t szLgDst);

/**
 * @brief Conversion de la chaine en majuscule
 *
 * @param[out] pcDst Chaine en majuscule
 * @param[in] pcSrc Chaine initiale
 * @param[in] szLgDst Taille maximale de la chaine en majuscule
 *
 * @return Chaine en majuscule (NULL en cas de probleme)
 */
char* STR_String2Upper(char* pcDst, const char* pcSrc, size_t szLgDst);

/**
 * @brief Padding a gauche d'une chaine de caractere
 *
 * @param[out] pcDst Chaine paddee
 * @param[in] pcSrc Chaine initiale
 * @param[in] szLgDst Taille maximale de la chaine paddee
 * @param[in] cPaddingChar Caractere de padding
 *
 * @return Chaine paddee (NULL en cas de probleme)
 */
char* STR_LeftPadding(char* pcDst, const char* pcSrc, size_t szLgDst, char cPaddingChar);

/**
 * @brief Padding a droite d'une chaine de caractere
 *
 * @param[out] pcDst Chaine paddee
 * @param[in] pcSrc Chaine initiale
 * @param[in] szLgDst Taille maximale de la chaine paddee
 * @param[in] cPaddingChar Caractere de padding
 *
 * @return Chaine paddee (NULL en cas de probleme)
 */
char* STR_RightPadding(char* pcDst, const char* pcSrc, size_t szLgDst, char cPaddingChar);

/**
 * @brief Centrage d'une chaine de caractere
 *
 * @param[out] pcDst Chaine centree
 * @param[in] pcSrc Chaine initiale
 * @param[in] szLgDst Taille maximale de la chaine centree
 * @param[in] cPaddingChar Caractere de padding
 *
 * @return Chaine centree (NULL en cas de probleme)
 */
char* STR_Center(char* pcDst, const char* pcSrc, size_t szLgDst, char cPaddingChar);

/**
 * @brief Duplication d'une chaine
 *        Cette fonction a un comportement identique a la fonction strdup() et permet de fournir une alternative fonctionnant sur toutes les cibles
 *        La chaine dupliquee ayant ete allouee via malloc(), l'appelant doit liberer cette chaine
 *
 * @param[in] pcSrc Chaine initiale
 *
 * @return Chaine dupliquee (NULL en cas de probleme)
 */
char* STR_StrDup(const char* pcSrc);

#ifdef __cplusplus
}
#endif

#endif
